// this is my arduino code, let fix it and give me full correct code : /* Copyright 2024 Chirale, TensorFlow Authors.
// ==============================================================================*/
...
// // If you stream int8 samples, Arduino dequantizes int8 -> float:
...
// // mbed CPU stats for active vs sleep
...
// // ==== Filter Settings =====
// constexpr float FS_HZ = 64.0f;   // MUST match your host stream Fs
...
// static volatile uint32_t g_hop = (uint32_t)HOP_DEFAULT;

// // ===== A2 experiment knobs: batch K windows before running inferences =====
// static volatile uint32_t g_batch_windows = 8;  // 1/2/4/8
// static uint32_t g_pending_infer = 0;          // due but not executed
// static uint32_t g_pending_peak = 0;           // peak backlog in current PWR window

// // Ring buffer
// static float ringbuf[RING];
// static uint32_t widx = 0;
// static uint32_t total_samples = 0;
// static uint32_t last_infer_sample = 0;
// static uint32_t missed_infer = 0;
...
//   const float RC = 1.0f / (2.0f * 3.1415926f * HP_HZ);
//   hp_a = RC / (RC + dt);
...
//   g_pending_peak = 0;  // reset peak per reporting window
//   init_cpu_stats();
// }
...
//   const uint32_t dt_ms = now - g_last_pwr_ms;
...
//   const uint64_t du_uptime = cur.uptime - g_cpu_prev.uptime;
//   const uint64_t du_sleep  = cur.sleep_time - g_cpu_prev.sleep_time;
//   const uint64_t du_deep   = cur.deep_sleep_time - g_cpu_prev.deep_sleep_time;

//   const uint64_t du_idle = du_sleep + du_deep;
//   const uint64_t du_active = (du_uptime > du_idle) ? (du_uptime - du_idle) : 0;

//   const float active_pct = (du_uptime > 0) ? (100.0f * (float)du_active / (float)du_uptime) : 0.0f;

//   const float dt_s = dt_ms / 1000.0f;
//   const float samp_s = (dt_s > 0) ? (g_sample_cnt / dt_s) : 0.0f;
//   const float inf_s  = (dt_s > 0) ? (g_infer_cnt  / dt_s) : 0.0f;

//   const float infer_ms_s = (dt_s > 0) ? ((g_us_infer / 1000.0f) / dt_s) : 0.0f;
//   const float fill_ms_s  = (dt_s > 0) ? ((g_us_fill  / 1000.0f) / dt_s) : 0.0f;
//   const float rx_ms_s    = (dt_s > 0) ? ((g_us_rx    / 1000.0f) / dt_s) : 0.0f;

//   const float Eproxy_ms_s = infer_ms_s + fill_ms_s + rx_ms_s;
...
//   Serial.print("BATCH="); Serial.print((uint32_t)g_batch_windows); Serial.print("  ");
//   Serial.print("pend_now="); Serial.print(g_pending_infer); Serial.print("  ");
//   Serial.print("pend_peak="); Serial.print(g_pending_peak); Serial.print("  ");
...
//   // reset window counters (keep backlog counters!)
//   g_us_infer = g_us_fill = g_us_rx = 0;
//   g_infer_cnt = g_sample_cnt = 0;

//   g_cpu_prev = cur;
//   g_last_pwr_ms = now;
//   g_pending_peak = 0; // reset peak for next reporting window
// }
...
//   const uint32_t end = widx; // exclusive
//   const uint32_t start = (end + RING - N) % RING;

//   for (int i = 0; i < N; i++) {
//     const uint32_t idx = (start + (uint32_t)i) % RING;
//     const float x = ringbuf[idx];
//     k++;
//     const float delta = x - m;
//     m += delta / (float)k;
//     const float delta2 = x - m;
//     M2 += delta * delta2;
//   }

//   mean = m;
//   const float var = (N > 1) ? (M2 / (float)(N - 1)) : 0.0f;
//   stdv = sqrtf(var);
// }
...
//   const float inv = 1.0f / (stdv + NORM_EPS);
...
//       const uint32_t idx = (start + (uint32_t)i) % RING;
//       const float x = ringbuf[idx];
//       const float z = (x - mean) * inv;
//       const int q = (int)lroundf(z / scale) + zp;
...
//   int n_out = 1;
//   for (int i = 0; i < model_output->dims->size; i++) n_out *= model_output->dims->data[i];
...
//   const uint32_t t_fill0 = micros();
//   if (!fill_input_last_window()) return false;
//   const uint32_t t_fill1 = micros();
...
// // Execute ALL pending inferences immediately (used for optional cleanup)
// static void flush_pending_infer() {
//   while (g_pending_infer > 0) {
//     run_inference_once();
//     g_pending_infer--;
//   }
// }

// // One decoded sample -> filter -> ring -> inference schedule (A2 batching)
// static void handle_sample_float(float v) {
//   g_sample_cnt++;

//   const float vf = hp1_filter(v);
//   ring_push(vf);

//   if (total_samples < (uint32_t)N) return;

//   uint32_t hop = (uint32_t)g_hop;
//   if (hop == 0) hop = 1;

//   // accumulate due windows
//   while ((total_samples - last_infer_sample) >= hop) {
//     const uint32_t due = total_samples - last_infer_sample;

//     // missed bookkeeping if we fell behind by >1 hop
//     if (due >= (2u * hop)) missed_infer += (due / hop) - 1u;

//     last_infer_sample += hop;
//     g_pending_infer++;
//     if (g_pending_infer > g_pending_peak) g_pending_peak = g_pending_infer;

//     uint32_t bw = (uint32_t)g_batch_windows;
//     if (bw == 0) bw = 1;

//     // if threshold reached: burst-run all pending
//     if (g_pending_infer >= bw) {
//       while (g_pending_infer > 0) {
//         run_inference_once();
//         g_pending_infer--;
//       }
...
//   switch (rx_state) {
//     case 0: if (b == SYNC1) rx_state = 1; break;
//     case 1: rx_state = (b == SYNC2) ? 2 : 0; break;
...
// // ===== Clean apply functions for settings =====
// static void apply_hop(uint32_t hop) {
//   g_hop = hop;

//   // IMPORTANT: avoid mixing old schedule with new hop
...
// static void apply_batch(uint32_t bw) {
//   g_batch_windows = bw;

//   // IMPORTANT: avoid mixing old pending with new setting
//   last_infer_sample = total_samples;
//   missed_infer = 0;
//   g_pending_infer = 0;
//   g_pending_peak = 0;
...
//   reset_power_counters();
//   Serial.print("OK BATCH "); Serial.println((uint32_t)g_batch_windows);
// }
...
//   if (strcmp(s, "FLUSH") == 0) { flush_pending_infer(); Serial.println("OK FLUSH"); return; }

//   // HOP <val>
//   if (strncmp(s, "HOP ", 4) == 0) {
//     int hop = atoi(s + 4);
//     if (hop == 512 || hop == 384 || hop == 256 || hop == 128) {
//       apply_hop((uint32_t)hop);
//     } else {
...
//   // BATCH <val>
//   if (strncmp(s, "BATCH ", 6) == 0) {
//     int bw = atoi(s + 6);
//     if (bw == 1 || bw == 2 || bw == 4 || bw == 8) {
//       apply_batch((uint32_t)bw);
//     } else {
//       Serial.println("ERR BATCH must be one of: 1 2 4 8");
//     }
//     return;
//   }
...
//   // Default: numeric sample (TEXT mode)
//   char* endptr = nullptr;
...
//   Serial.print("Model bytes: "); Serial.println(cnn_1d_float_len);
//   Serial.print("Model KB: "); Serial.println(cnn_1d_float_len / 1024.0f, 2);
...
//   Serial.print("Tensor arena bytes: "); Serial.println(kTensorArenaSize);
...
//   Serial.println("  RUN");
//   Serial.println("  RST");
//   Serial.println("  FLUSH                 (optional)");
//   Serial.println("  HOP 512|384|256|128");
//   Serial.println("  BATCH 1|2|4|8");
//   Serial.println("  MODE TEXT | MODE BIN_F32 | MODE BIN_I8");
//   Serial.println("  I8Q <scale> <zp>");
...
//     }
//   }
//   const uint32_t rx1 = micros();
//   g_us_rx += (uint32_t)(rx1 - rx0);
// }


/* Copyright 2024 Chirale, TensorFlow Authors.
==============================================================================*/
...
enum StreamMode { STREAM_TEXT=0, STREAM_BIN_F32=1, STREAM_BIN_I8=2 };
static StreamMode g_stream_mode = STREAM_BIN_F32;
...
static uint32_t g_last_pwr_ms = 0;
// default 5s, but you can change at runtime with: PWRPERIOD <ms>
static uint32_t g_pwr_period_ms = 500;
...
// ===== Profiling output gates (IMPORTANT for A1/A2 stability) =====
// Flooding Serial TX with PROB/PRED/MEM often prevents [PWR] from being observed.
static bool g_print_pred = false;  // PROB/PRED/OLED per inference
static bool g_print_mem  = false;  // [MEM] periodic + after_infer
...
static void report_power_energy() {
  ...
  if (g_last_pwr_ms == 0) { g_last_pwr_ms = now; return; }
  const uint32_t dt_ms = now - g_last_pwr_ms;
  if (dt_ms < g_pwr_period_ms) return;
  ...
  // IMPORTANT: keep [PWR] format exactly aligned with your Python regex
  // (no extra fields between BATCH and pend_peak; no "pend_now=")
  Serial.print("[PWR] dt="); Serial.print(dt_s, 2); Serial.print("s  ");
  Serial.print("Fs="); Serial.print(FS_HZ, 1); Serial.print("Hz  ");
  Serial.print("HOP="); Serial.print((uint32_t)g_hop); Serial.print("  ");
  Serial.print("BATCH="); Serial.print((uint32_t)g_batch_windows); Serial.print("  ");
...
static void apply_hop(uint32_t hop) {
  ...
  // avoid mixing old schedule with new hop
  last_infer_sample = total_samples;
  missed_infer = 0;
  g_pending_infer = 0;
  g_pending_peak = 0;
...
static void apply_batch(uint32_t bw) {
  ...
  // avoid mixing old pending with new setting
  last_infer_sample = total_samples;
  missed_infer = 0;
  g_pending_infer = 0;
  g_pending_peak = 0;
...
static void handle_line(char* s) {
  ...
  // ==== Output gating (IMPORTANT for profiling) ====
  if (strcmp(s, "PRED ON") == 0)  { g_print_pred = true;  Serial.println("OK PRED ON");  return; }
  if (strcmp(s, "PRED OFF") == 0) { g_print_pred = false; Serial.println("OK PRED OFF"); return; }
  if (strcmp(s, "MEM ON") == 0)   { g_print_mem  = true;  Serial.println("OK MEM ON");   return; }
  if (strcmp(s, "MEM OFF") == 0)  { g_print_mem  = false; Serial.println("OK MEM OFF");  return; }

  // PWRPERIOD <ms>
  if (strncmp(s, "PWRPERIOD ", 10) == 0) {
    int ms = atoi(s + 10);
    if (ms < 200) ms = 200;
    g_pwr_period_ms = (uint32_t)ms;
    reset_power_counters();
    Serial.print("OK PWRPERIOD "); Serial.println(g_pwr_period_ms);
    return;
  }
...
void setup() {
  ...
  Serial.println("  MODE TEXT | MODE BIN_F32 | MODE BIN_I8");
  Serial.println("  I8Q <scale> <zp>");
  Serial.println("  PRED ON|OFF       (default OFF for profiling)");
  Serial.println("  MEM ON|OFF        (default OFF for profiling)");
  Serial.println("  PWRPERIOD <ms>    (default 5000)");
...
void loop() {
  ...
  // periodic memory print (every 2s) ONLY if MEM ON
  if (g_print_mem && (now - g_last_mem_print_ms >= 2000)) {
    g_last_mem_print_ms = now;
    reportMemorySerial("periodic");
  }

  // power/energy report (every g_pwr_period_ms)
  report_power_energy();
